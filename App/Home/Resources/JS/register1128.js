...
$('#regtel').focus(function(){
	...
	$(this).siblings('span').html('推荐使用手机注册，购物更安全');
	}).blur(function(){
		var user=$(this).val();
		if(user==""){
		...
		if (user.indexOf("@") > -1) {
			...
			else 	$(this).siblings('span').html('邮箱格式错误').addClass('blurtip');
			...
			else 	$('#regtel').siblings('span').html('手机或邮箱格式错误').addClass('blurtip');
...
$('#psdrepeat').focus(function(){
	...
	var psw=$('#regpsd').val();
	var pswR=$("#psdrepeat").val();
	
	if(psw != pswR)
		$('#psdrepeat').siblings('span').html('两次密码不匹配').addClass('blurtip');
	else if( psw!=''&&pswR!='' ) {
...
$('.rg_button').click(function(){
	alert("hello");
	  ...
	  if($('#psdrepeat').attr("check")=="true"&&$('#regtel').attr("check")=="true"&&$('#rg2014_agreement').attr("checked")=="checked"){
		  alert($('#psdrepeat').attr("check"));
		  alert($('#regtel').attr("check"));
	  ...
	  $('#showagree').click(function(){
			...
			var $oa1=$( '<a href="#" id="closethickbox"></a>' );
