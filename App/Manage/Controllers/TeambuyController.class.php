...
class TeambuyController extends Controller {
	...
	function teambuy(){
		...
		$teambuy = new Teambuy();
		$teambuyData = $teambuy->getTeambuy();
		
		if($teambuyData){
			
			$this->assign('teambuy',$teambuyData);
	...
	function audit(){
		...
		if (!in_array($status, array('10000','20000'))){
			echo '<br>数据不可修改.<a href="teambuy">返回</a><br>';
			return ;
		}
		...
		$m->setTable ( 'vcb_index_team_buy' );					//设置表名
		$m->setWhere('product_id','=',$id);				//设置Where条件
		$m->update();
		
		//返回
		echo '<br>操作成功,<a href="teambuy" >返回</a><br>';	
	}
	...
	function update(){
		...
		$teambuy = new Teambuy();
		$updateData = $teambuy->getUpdateData($id);
...
function add_save(){
		...
		if (!$product_id){										
			...
			echo '<br>保存成功，<a href="add">继续添加</a>,<a href="teambuy">返回</a><br>';
		}
	}
	/**
	 * 上传图片
	...
	function upload(){
		$upload = new Upload();
		$data =  $upload->save('Index/IndexTeamBuy');
