...
class SurveyController extends Controller {
    ...
    public function add_save() {
        ...
        $title = trim(get_post_value('survey_title'));
        $field = array(
            'title' => $title,
    ...
    public function add_issue_save() {
        $id = get_post_value('id');
        if (trim(get_post_value('issue_title')) != null) {
            $issue = trim(get_post_value('issue_title'));
            $field = array(
                'survey_id' => $id,
                'issue' => $issue,
                'status' => '10000',
            ...
            $issue = trim(get_post_value('issue_title2'));
            $field = array(
                'survey_id' => $id,
                'issue' => $issue,
                'status' => '10000',
    ...
    public function update_issue_save() {
        $id = get_post_value('id');
        $issue = trim(get_post_value('issue'));
        $field = array(
            'issue' => $issue,
        );
        $m = new Survey();
    ...
    public function count_re() {
        ...
        for ($i = $star; $i <= $end; $i++) {
            //操作other
            if (!empty(trim(get_post_value('other' . $i)))) {
                $content = trim(get_post_value('other' . $i));
                /* echo $i.$other_content; */
                //$this->count_item($i);
                $this->re_other($i, $content);
                $this->record(0);
            }
            //end 操作

            if (!empty(get_post_value($i))) {
    ...
    //记录数据到survey_rerecord private

    private function record($i) {
        $field = array(
            'item_id' => $i,
            'user_name' => '',
        ...
        $m->clear();
        $m->setField($field);
        $m->setTable('vcb_survey_rerecord');
        $m->insert();
    }

    //客户建议记录到survey_other private

    private function re_other($i, $content) {
        ...
        $m->clear();
        $m->setField($field);
        $m->setTable('vcb_survey_other');
        $m->insert();
    }

    //other_stat頁面顯示
    public function other_stat() {
        $id = get_post_value('id');
        $field = array(
            'id',
            'issue_id',
